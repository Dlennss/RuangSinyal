...
function agentName(item: AgentCreditApplication) {
  return applicantValue(item, "agent_name", item.member_name || item.agent_name || "Agent RuangSinyal");
}
...
export function MarketingReportCenter({ applications }: Props) {
  ...
  const rows = useMemo<ReportRow[]>(() => {
    if (tab === "agents") {
      ...
      return Array.from(latest.values()).map((item) => ({
        ...
        Tier: item.credit_level_name || "RuangSinyal Start",
  ...
  async function downloadPdf() {
    ...
    try {
      ...
      doc.setFontSize(16);
      doc.text(`RuangSinyal - ${reportTitle}`, 32, 34);
      doc.setFontSize(9);
