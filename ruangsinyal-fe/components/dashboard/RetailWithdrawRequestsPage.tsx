              ...
              <tbody className="divide-y divide-slate-100 text-sm">
                {items.map((item) => (
                  <tr key={item.id} className="align-top">
                    <td className="px-4 py-4"><p className="font-black">{item.member_nama || "Agent RuangSinyal"}</p><p className="mt-1 text-[10px] font-semibold text-slate-500">{item.member_email || item.ref_id}</p></td>
          ...
          <div className="divide-y divide-slate-100 md:hidden">
            {items.map((item) => (
              <article key={item.id} className="p-4">
                <div className="flex items-start justify-between gap-3"><div><p className="text-sm font-black">{item.member_nama || "Agent RuangSinyal"}</p><p className="mt-1 text-[10px] font-bold text-sky-700">{sourceLabel(item.source_type)}</p></div><p className="text-sm font-black text-sky-700">{fmtIDR(item.amount)}</p></div>
