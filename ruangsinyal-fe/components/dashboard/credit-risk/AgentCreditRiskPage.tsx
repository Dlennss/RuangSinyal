...
function buildAgentSummaries(applications: AgentCreditApplication[]): AgentSummary[] {
  ...
  return Array.from(grouped.entries()).map(([memberId, items]) => {
    ...
    return {
      memberId,
      name: applicantText(latest, "agent_name", latest.member_name || latest.agent_name || "Agent RuangSinyal"),
      ...
      currentLevel: latest.credit_level_name || "RuangSinyal Start",
                          ...
                          <div className="mt-3 grid gap-3 lg:grid-cols-[1fr_220px]">
                            <label className="block rounded-2xl border border-sky-950/[0.06] bg-white p-3">
                              ...
                              <textarea
                                ...
                                placeholder={mode === "operator" ? "Contoh: pembayaran lancar, naik ke RuangSinyal Plus." : "Contoh: agent ramai dan layak diajukan naik limit."}
                              />
                            </label>
