...
function getPaymentHelper(methodId: string, applicationId: number) {
  if (methodId === "qris") {
    return {
      ...
      desc: "Scan QRIS dari kanal RuangSinyal, lalu upload bukti pembayaran.",
      rows: [
  ...
  if (methodId === "transfer") {
    return {
      ...
      rows: [
        ["Bank", "BCA"],
        ["Atas Nama", "RuangSinyal"],
      ],
    };
  }
  return {
    title: "Penagihan Offline",
    value: "Marketing RuangSinyal",
...
function getApplicantLabel(item: AgentCreditApplication) {
  ...
  return {
    name: String(data.agent_name || data.nama_lengkap || "Agent RuangSinyal"),
...
export function UserCreditBillPayLaterContent({ bills }: Props) {
  ...
  async function copyPaymentValue() {
    if (!paymentHelper.value || paymentHelper.value === "Saldo akun RuangSinyal") return;
