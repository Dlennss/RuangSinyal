...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!brand) {
    return buildPageMetadata({
      title: "HP Pascabayar | RuangSinyal",
      description: "Cek dan bayar tagihan HP pascabayar di RuangSinyal.",
      path: "/hp-pascabayar",
    });
  }

  return buildPageMetadata({
    title: `${toTitleCase(brand.nama)} | HP Pascabayar RuangSinyal`,
    ...
    keywords: [`${String(brand.nama).toLowerCase()} pascabayar`, `bayar ${String(brand.nama).toLowerCase()}`, "hp pascabayar", "ruangsinyal"],
  });
}
...
export default async function HPPascabayarSlugPage({ params }: PageProps) {
  ...
  const collectionJsonLd = buildCollectionJsonLd({
    title: `${brandTitle} | HP Pascabayar RuangSinyal`,
