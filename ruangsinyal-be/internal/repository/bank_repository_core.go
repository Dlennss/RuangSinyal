...
const (
	...
	RuangSinyalDepositBankName      = "BNI"
	RuangSinyalDepositAccountNumber = "1955637480"
	RuangSinyalDepositAccountHolder = "M Sansan Irfanda"
)
...
// EnsureRuangSinyalDepositBank keeps the public deposit account available after
// deployment, including installations where SQL migrations are applied later.
func (r *BankRepository) EnsureRuangSinyalDepositBank(ctx context.Context) error {
	if r == nil || r.db == nil {
...
`, RuangSinyalDepositAccountNumber, RuangSinyalDepositBankName, RuangSinyalDepositAccountHolder)
	if err != nil {
		return err
	}
...
SET nomor_rekening = EXCLUDED.nomor_rekening,
    ...
    admin_staff_only = false,
    diubah_pada = now()
`, RuangSinyalDepositBankName, RuangSinyalDepositAccountNumber, RuangSinyalDepositAccountHolder)
	return err
}
